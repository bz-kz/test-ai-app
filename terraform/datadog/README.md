...
6. **State 移行** — ローカル tfstate を HCP へ uploads:
   ...
   terraform init    # HCP migration: "Do you want to copy existing state to HCP Terraform?" に yes
   ```
...
- Backend: HCP Terraform (`test-ai-app-datadog` workspace)。Operator セットアップ手順は上の "CI/CD" 章 step 1-8 参照。HCP 未移行時はローカル `terraform.tfstate` (上の "使い方" 章) を使う。
