...
- **Acceptance:**
  ...
  - [ ] Infrastructure mounts (`src/components/_<name>/`): `_` prefix で private folder 化された一群。Atomic Design 三層 (atoms/molecules/organisms) には属さず、layout から 1 箇所だけ mount される client island の置き場。現状 `src/components/_rum/RumInit.tsx` (ADR-0006 FE-015 — Datadog Browser RUM init) のみ。新規追加には ADR 必要。
