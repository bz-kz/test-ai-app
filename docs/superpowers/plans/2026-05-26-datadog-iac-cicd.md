...
> **重要**: このタスクは外部状態 (HCP Terraform organization、GH Actions secret、ローカル `terraform init -migrate-state`) を変更します。エージェントが自動実行せず、**人間オペレータが手動で実行** してください。
...
# プロンプト "Do you want to copy existing state to the new backend?" に yes
```
