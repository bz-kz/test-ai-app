...
| **G1** | Type                    | Generator                                          | 0 errors.                                                                                                                                         | Frontend: `cd frontend && pnpm typecheck`. Backend: `cd backend && pyright`.                                                                                                              |
| **G2** | Lint / Format           | Generator                                          | 0 errors. Format clean.                                                                                                                           | Frontend: `pnpm lint && pnpm format:check`. Backend: `ruff check . && ruff format --check .`.                                                                                             |
| **G3** | Unit                    | Generator                                          | All affected unit tests green. New behaviour has at least one test. Inference layer mocked via `FakeLocalLLMClient`.                              | Frontend: `pnpm test -- --run`. Backend: `pytest -q`.                                                                                                                                     |
