...
- **Acceptance:**
  ...
  - [ ] Timeout default: 600 s for `generate` and 1200 s end-to-end for `stream` (raised from 300 s / 300 s in PR #13 / INF-006 to cover the measured 4-5 min CPU envelope; previously raised from 60 s / 120 s in commit `fa04bae`). The stream timeout is `LLM_TIMEOUT_S × 2` per `ollama_client.stream()`. Cancellable.
...
- **Acceptance:**
  ...
  - [ ] Latency budget for `gemma4:e4b` first-token: p95 ≤1 s; total response p95 ≤6 s for 1k output tokens. **This is the aspirational GPU target.** On the CPU-only reference path the measured operating point is first-token ~2–3 min and total ~4–5 min (Playwright reproduction 2026-05-13, PR #13 / INF-006); the gap is structural to CPU inference on `gemma4:e4b` Q4_K_M and cannot be closed at the application layer. Closing it requires a separate ADR (GPU offload, hosted LLM under a revised `local-llm-and-phi.md` §1, model swap, or quantization A/B). The UI MUST treat the CPU operating point as the current truth (see `frontend/SPEC.md#latency-ux-budget` + `DRAFT_GENERATION_ETA_MS=240_000`) so the system is usable while the gap stands.
