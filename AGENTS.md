...
- Never `git push origin main` (any form). `main` is a GitHub-protected branch (configured 2026-05-13); the rule is enforced both by agent discipline and server-side. Pushing to non-default feature branches and opening PRs via `gh pr create` is permitted (the human merges) — see §8.7.
- Never `git push --force` / `--force-with-lease` on any branch, including the agent's own feature branch. History rewrite remains human-only. Force-push to `main` is additionally blocked server-side.
...
- Never `gh pr merge` in any form (`--auto`, `--admin`, `--squash`, `--rebase`, `--merge`). Never `gh pr close` / `gh pr reopen` / `git push --delete`. Merge is the human's sole responsibility — the agent hands the PR off and stops. See §8.7 for full PR-flow rules.
